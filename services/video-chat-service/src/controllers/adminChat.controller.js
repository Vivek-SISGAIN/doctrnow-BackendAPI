    ...
    try {
        ...
        if (!ALLOWED_REQUESTER_ROLES.includes(role)) {
            return res.status(403).json({
                success: false,
                error: "Only Hospital Admins, Doctors, and Patients can create chat requests",
            });
        }
...
const acceptRequest = async (req, res, next) => {
    try {
        ...
        const requesterId = session.hospitalAdminId || session.doctorId || session.patientId;
        if (requesterId) {
...
const uploadAttachment = async (req, res, next) => {
    try {
        ...
        if (!ALLOWED_TYPES.includes(mimetype)) {
            return res.status(415).json({ success: false, error: "File type not supported." });
        }
        ...
        // Upload to S3
        const { s3Client, S3_BUCKET, generatePresignedUrl } = require("../config/s3");
