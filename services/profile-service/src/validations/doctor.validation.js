...
const createDoctorSchema = Joi.object({
  ...
  primarySpecialization: Joi.string().min(2).max(100).required()
    .messages({
  ...
  medicalDegree: Joi.string().min(2).max(100).required()
    .messages({
  ...
  professionalBio: Joi.string().min(50).max(2000).required()
    .messages({
...
const updateDoctorSchema = Joi.object({
  ...
  licenseType: Joi.string().valid('FULL_LICENSE', 'TEMPORARY_LICENSE', 'SPECIALIST_LICENSE', 'CONSULTANT_LICENSE').optional(),
  ...
  workingHoursFrom: Joi.string().pattern(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/).optional(),
  workingHoursTo: Joi.string().pattern(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/).optional(),
