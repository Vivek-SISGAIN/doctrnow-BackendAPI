...
const createHospitalAdminSchema = Joi.object({
  ...
  hospitalName: Joi.string().min(3).max(200).required()
    .messages({
      'any.required': 'Hospital name is required'
    }),
  hospitalId: Joi.string().min(3).max(50).required()
    .messages({
...
const updateHospitalAdminSchema = Joi.object({
  ...
  hospitalName: Joi.string().min(3).max(200).optional(),
  hospitalId: Joi.string().min(3).max(50).optional(),
