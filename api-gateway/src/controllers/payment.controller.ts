 ...
 * Target: payment-insurance-service (/api/payments)
...
export class PaymentController {
  ...
  /** Base path: POST /api/v1/payments, GET /api/v1/payments, etc. */
  @All()
  @ApiOperation({ summary: 'Proxy payment base requests to payment-insurance-service' })
  async proxyBase(@Req() req: Request, @Res() res: Response): Promise<void> {
    return this.proxyRequest(req, res);
  }

  /** Subpaths: /api/v1/payments/:id, /api/v1/payments/:id/refund, etc. */
  @All('*')
  @ApiOperation({ summary: 'Proxy payment subpath requests to payment-insurance-service' })
  async proxyRequest(@Req() req: Request, @Res() res: Response): Promise<void> {
    ...
    const suffix = incomingPath.replace(/^\/api\/v1\/payments/, '') || '';
    const path = `/api/payments${suffix}`;
