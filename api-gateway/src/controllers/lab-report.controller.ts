 ...
 * Routes: /api/v1/lab-reports/*
 * Target: medical-records-service /api/lab-reports
 * Schema matches doctor portal UI (LabReportDetail, TestResult, etc.)
 */
@ApiTags('lab-reports')
...
export class LabReportController {
  ...
  async proxyRequest(@Req() req: Request, @Res() res: Response): Promise<void> {
    ...
    const suffix = incomingPath.replace(/^\/api\/v1\/lab-reports/, '') || '';
    const path = `/api/lab-reports${suffix}`;
